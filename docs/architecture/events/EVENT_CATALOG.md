...
**Status:** Implementado — producer entregue na Story #32; consumer de reconhecimento entregue na Story #33; confiabilidade inicial na Story #34 e visibilidade operacional local na Story #44
...
| Consumer | Inventory Service — registra processamento idempotente por `eventId`, recupera localmente pendências temporárias, persiste a evidência de reconhecimento e disponibiliza observação operacional local por `orderId`; não altera estoque. |
...
| Observações | A publicação usa `acks=all`. REST continua ativo; o consumidor não executa reserva, baixa ou outra regra comercial. A recuperação interna da Story #34 e a observação local da Story #44 não mudam o contrato do evento. |
