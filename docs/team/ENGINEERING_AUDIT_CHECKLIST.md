...
| Governance | **Sprint Initiation Request:** a published request exists before Sprint Bootstrap, follows the institutional template, identifies the Sprint, objective, initial Story or materialization direction, base branch, boundaries, institutional references and explicit authorization to begin. | |
| Governance | **PMO Bootstrap:** the PMO evidence and initial `STATUS.md` show that Sprint Bootstrap occurred only after the authorized Sprint Initiation Request and produced the PMO → Product Owner handoff. | |
