...
| Governance | **Sprint opening sequence:** published evidence shows, in order, Organizational Validation, effective Organizational Freeze, Sprint Initiation Request, PMO Sprint Bootstrap, initial `STATUS.md` and Product Owner entry. | |
| Governance | **Sprint Initiation Request:** after Organizational Validation and Organizational Freeze, a published request exists before Sprint Bootstrap, follows the institutional template, identifies the Sprint, objective, initial Story or materialization direction, base branch, boundaries, institutional references and explicit authorization to begin. | |
| Governance | **PMO Bootstrap:** the PMO evidence and initial `STATUS.md` show that Sprint Bootstrap occurred only after the validated, frozen and authorized opening state, and produced the PMO → Product Owner handoff. | |
